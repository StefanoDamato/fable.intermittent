test_that("NEGBINES fits, forecasts, and generates on all test series", {
  ...
  for (s in series_list) {
    ...
    # Fit model
    fit <- fabletools::model(data, model = NEGBINES(value))
    expect_s3_class(fit, "mdl_df")
    expect_identical(fabletools::model_sum(fit$model[[1]]), "NEGBINES")
