for (i in 1:length(test_data)){
  for (damped in c(TRUE, FALSE)) {
    test_that(paste0("NEGBINES ", ifelse(damped, "(damped) ", "(undamped) "), 
                     "fits, forecasts, and generates on t.s. ", i), {
      test_ts <- test_data[[i]]
      
      # Check that the model fits correctly
      expect_no_error({
      fit <- fabletools::model(test_ts, model = NEGBINES(value, damped = damped))
      })
      expect_s3_class(fit, "mdl_df")
      expect_match(fabletools::model_sum(fit$model[[1]]), "^NEGBINES\\((d|u)\\)$")
      if (!damped) expect_identical(fabletools::model_sum(fit$model[[1]]), "NEGBINES(u)")
...
test_that("NEGBINES validates inputs and rejects unsupported options", {
  expect_error(
    fable.intermittent:::train_negbines(multivariate_ts(), specials = list(), damped = TRUE),
    "Only univariate responses"
  )
  expect_error(
    fable.intermittent:::train_negbines(all_na_ts(), specials = list(), damped = TRUE),
    "All observations are missing"
  )
  expect_error(
    fable.intermittent:::train_negbines(some_na_ts(), specials = list(), damped = TRUE),
    "Missing values are not supported"
  )
  expect_error(
    fable.intermittent:::train_negbines(base_ts(), specials = list(), damped = "yes"),
    "`damped` must be a boolean"
  )
  expect_error(
    fable.intermittent:::negbines_no_xreg(),
    "Exogenous regressors are not supported"
  )

  fit <- fabletools::model(base_ts(), NEGBINES(value))
  expect_error(
