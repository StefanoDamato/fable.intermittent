test_that("VZ fits, forecasts, and generates on all test series", {
  set.seed(123)
  ...
  for (s in series_list) {
    ...
    # Fit model
    fit <- fabletools::model(data, model = VZ(value))
    expect_s3_class(fit, "mdl_df")
    expect_identical(fabletools::model_sum(fit$model[[1]]), "VZ")
