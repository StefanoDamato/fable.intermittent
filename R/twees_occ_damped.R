...
#' @export
TWEES_OCC_DAMPED <- function(formula, damped = TRUE, scaling = TRUE, ...) {
  twees_model <- new_model_class(
    "TWEES_OCC_DAMPED",
    train = train_twees_OCC_DAMPED,
    specials = new_specials(
      xreg = twees_occ_damped_no_xreg
    )
  )
...
#' @importFrom stats median
train_twees_OCC_DAMPED <- function(.data, specials, damped, scaling, ...) {
  if (length(measured_vars(.data)) > 1) {
    abort("Only univariate responses are supported by TWEES_OCC_DAMPED.")
  }
  ...
  if (anyNA(y)) {
    abort("Missing values are not supported by TWEES_OCC_DAMPED.")
  }
  ...
  opt <- twees_occ_damped_optimize(y_scaled, occurrence, damped)
  x <- opt$solution
  rho <- x[1]
  p0 <- x[2]
  alpha_p <- x[3]
...
#'
#' @export
forecast.TWEES_OCC_DAMPED <- function(object, new_data, specials = NULL, times = 10000, ...) {
  h <- nrow(new_data)
...
#' @export
generate.TWEES_OCC_DAMPED <- function(x, new_data, specials = NULL, ...) {
  h <- nrow(new_data)
  sim <- twees_occ_damped_simulate(x, h, times = 1)
...
#' fitted(fit)
#' @export
fitted.TWEES_OCC_DAMPED <- function(object, ...) {
  object$fitted
}
...
#' residuals(fit)
#' @export
residuals.TWEES_OCC_DAMPED <- function(object, ...) {
  object$residuals
}


#' @export
model_sum.TWEES_OCC_DAMPED <- function(x) {
  if (x$theta_mu != 0) "TWEES_OCC_DAMPED(d)" else "TWEES_OCC_DAMPED(u)"
}

#' @export
tidy.TWEES_OCC_DAMPED <- function(x, ...) {
...
#' @rdname TWEES
#' @export
report.TWEES_OCC_DAMPED <- function(object, ...) {
...
twees_occ_damped_simulate <- function(object, h, times) {
...
twees_occ_damped_optimize <- function(y, occ, damped) {
  ...
  if (!damped) {
    ...
    eval_f <- function(x) twees_nll(c(x[1:3], 0, x[4:5], 0), y, occ)
  } else {

    # Otherwise, learn the damping parameter with a simplex parametrisation
    init_params <- c(1.5, min(max(mean(occ), .TWEES_EPSILON), 1 - .TWEES_EPSILON), 0.2, 0.1 / (1 - 0.2), max(mean(y), .TWEES_EPSILON), 0.2, 0.1 / (1 - 0.2))
    lb <- c(1.2 + .TWEES_EPSILON, rep(.TWEES_EPSILON, 6))
    ub <- c(1.8 - .TWEES_EPSILON, rep(1 - .TWEES_EPSILON, 3), max(y) * 10, rep(1 - .TWEES_EPSILON, 2))
    eval_f <- function(x) twees_nll(c(x[1:3], (1 - x[3]) * x[4], x[5:6], (1 - x[6]) * x[7]), y, occ)
  }
  ...
  opt$solution <- if (damped) c(
    opt$solution[1:3], (1 - opt$solution[3]) * opt$solution[4],
    opt$solution[5:6], (1 - opt$solution[6]) * opt$solution[7]
  ) else c(opt$solution[1:3], 0, opt$solution[4:5], 0)
  opt
}

twees_occ_damped_no_xreg <- function(...) {
