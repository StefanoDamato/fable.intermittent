#' Tweedie Exponential Smoothing with occurrence damping and fixed p0
#'
#' As `TWEES_OCC_DAMPED`, except the occurrence process's initial state `p0`
#' is fixed at `clamp(mean(occurrence))` instead of being estimated by the
#' optimiser -- one fewer free parameter.
#'
...
#'
#' fc_ts <- ts |>
#'   model(TWEES_OCC_DAMPED_p0_FIXED(value)) |>
#'   forecast(h = "7 days")
#'
#' fc_ts |> print()
#'
...
#' @export
TWEES_OCC_DAMPED_p0_FIXED <- function(formula, damped = TRUE, scaling = TRUE, ...) {
  twees_model <- new_model_class(
    "TWEES_OCC_DAMPED_p0_FIXED",
    train = train_twees_OCC_DAMPED_p0_FIXED,
    specials = new_specials(
      xreg = twees_occ_damped_p0_fixed_no_xreg
    )
  )
...
#' @importFrom stats median
train_twees_OCC_DAMPED_p0_FIXED <- function(.data, specials, damped, scaling, ...) {
  if (length(measured_vars(.data)) > 1) {
    abort("Only univariate responses are supported by TWEES_OCC_DAMPED_p0_FIXED.")
  }
  ...
  if (anyNA(y)) {
    abort("Missing values are not supported by TWEES_OCC_DAMPED_p0_FIXED.")
  }
  ...
  opt <- twees_occ_damped_p0_fixed_optimize(y_scaled, occurrence, damped)
  x <- opt$solution
  rho <- x[1]
  p0 <- x[2]
  alpha_p <- x[3]
  theta_p <- if (damped) x[4] else 0
  mu0 <- x[5]
  alpha_mu <- x[6]
  theta_mu <- if (damped) x[7] else 0
  ...
  structure(
    ...
    class = "TWEES_OCC_DAMPED_p0_FIXED"
  )
}

#' Forecast a TWEES_OCC_DAMPED_p0_FIXED model
#'
#' Produces forecast distributions from a fitted TWEES_OCC_DAMPED_p0_FIXED
...
#'   index = time
#' )
#' fit <- model(ts, TWEES_OCC_DAMPED_p0_FIXED(value))
#' forecast(fit, h = "7 days")
#'
#' @export
forecast.TWEES_OCC_DAMPED_p0_FIXED <- function(object, new_data, specials = NULL, times = 10000, ...) {
  h <- nrow(new_data)
  ...
  sim <- twees_occ_damped_p0_fixed_simulate(object, h, times)
...
#' Generate sample paths from a TWEES_OCC_DAMPED_p0_FIXED model
#'
#' @param x A fitted `TWEES_OCC_DAMPED_p0_FIXED` model object.
#' @inheritParams forecast.TWEES_OCC_DAMPED_p0_FIXED
#'
...
#' @export
generate.TWEES_OCC_DAMPED_p0_FIXED <- function(x, new_data, specials = NULL, ...) {
  h <- nrow(new_data)
  sim <- twees_occ_damped_p0_fixed_simulate(x, h, times = 1)
  new_data$.sim <- as.numeric(sim[1, ])
  new_data
}

#' Extract fitted values from a TWEES_OCC_DAMPED_p0_FIXED model
#'
...
#' fitted(fit)
#' @export
fitted.TWEES_OCC_DAMPED_p0_FIXED <- function(object, ...) {
  object$fitted
}

#' Extract residuals from a TWEES_OCC_DAMPED_p0_FIXED model
#'
...
#' residuals(fit)
#' @export
residuals.TWEES_OCC_DAMPED_p0_FIXED <- function(object, ...) {
  object$residuals
}


#' @export
model_sum.TWEES_OCC_DAMPED_p0_FIXED <- function(x) {
  if (x$theta_mu != 0) "TWEES_OCC_DAMPED_p0_FIXED(d)" else "TWEES_OCC_DAMPED_p0_FIXED(u)"
}

#' @export
tidy.TWEES_OCC_DAMPED_p0_FIXED <- function(x, ...) {
...
#' @rdname TWEES_OCC_DAMPED_p0_FIXED
#' @export
report.TWEES_OCC_DAMPED_p0_FIXED <- function(object, ...) {
...
twees_occ_damped_p0_fixed_simulate <- function(object, h, times) {
...
twees_occ_damped_p0_fixed_optimize <- function(y, occ, damped) {

  # p0 is fixed, not estimated -- clamp(mean(occurrence)), same convention used
  # for the other models' initial values.
  p0_fixed <- min(max(mean(occ), .TWEES_EPSILON), 1 - .TWEES_EPSILON)

  # Define the function to be optimised. Kept as a 7-slot (rho, p0, alpha_p,
  # theta_p, mu0, alpha_mu, theta_mu) signature, matching TWEES_OCC_DAMPED's
  # twees_nll exactly, so p0 is simply held fixed at p0_fixed rather than
  # threaded through as a free parameter.
  ...
  # Re-expand the solution into the full 7-slot (rho, p0, alpha_p, theta_p,
  # mu0, alpha_mu, theta_mu) layout used by train_twees_OCC_DAMPED_p0_FIXED,
  # re-inserting the fixed p0 at slot 2.
  opt$solution <- if (damped) c(
    opt$solution[1], p0_fixed, opt$solution[2], (1 - opt$solution[2]) * opt$solution[3],
    opt$solution[4:5], (1 - opt$solution[5]) * opt$solution[6]
  ) else c(opt$solution[1], p0_fixed, opt$solution[2], 0, opt$solution[3:4], 0)
  opt
}

twees_occ_damped_p0_fixed_no_xreg <- function(...) {
  abort("Exogenous regressors are not supported by TWEES_OCC_DAMPED_p0_FIXED.")
}
